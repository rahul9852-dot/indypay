...
import {
  ...
  IExternalPayinPaymentRequestNxt,
  IExternalPayinPaymentResponseNxt,
  IExternalFyntraPayPayinRequest,
  IExternalFyntraPayPayinResponse,
...
export class PaymentsService {
  ...
  async createPayoutKDS(createPayoutDto: CreatePayoutDto, user: UsersEntity) {
    ...
    try {
      ...
      // Add to processing queue
      await this.payoutQueueKDS.add("process-payouts-kds-payout", {
        payoutOrders,
        userId: user.id,
        batchId,
      });
  ...
  async createJioPayin(
    ...
    // Call external TPI API **before starting transaction**
    const axiosServiceTpi = new AxiosService(TPI.BASE_URL);
    const payload: IExternalPayinPaymentRequestTPIPay = {
      amount,
      client_id: orderId,
      api_token: tpipay.apiToken,
      callback_url: tpipay.callbackUrl,
      customer_name: name,
      customer_mobile: mobile,
      customer_email: email,
      redirect_url: tpipay.redirectUrl,
    };
    ...
    try {
      const tpiResponse =
        await axiosServiceTpi.postRequest<IExternalPayinPaymentResponseTPI>(
          TPI.PAYIN.LIVE,
          payload,
        );

      if (!tpiResponse || tpiResponse.status !== "success") {
        this.logger.error(
          `PAYIN - createTransaction - TPI API failed`,
          tpiResponse,
        );
        throw new BadRequestException(
          new MessageResponseDto("Something went wrong"),
  ...
  async createFyntraPayin(
    ...
    // Call external TPI API **before starting transaction**
    const axiosServiceFyntra = new AxiosService(FYNTRA.BASE_URL);
    const payload: IExternalFyntraPayPayinRequest = {
      order_id: orderId,
      amount,
      customer_details: {
        name,
    ...
    let txnRefId: string | undefined;
    try {
      const fyntraResponse =
        await axiosServiceFyntra.postRequest<IExternalFyntraPayPayinResponse>(
          FYNTRA.PAYIN.LIVE,
          payload,
          {
            headers: {
              "X-API-KEY": fyntra.apiToken,
              Accept: "application/json",
            },
          },
        );
      if (!fyntraResponse || !fyntraResponse.status) {
        this.logger.error(
          `PAYIN - createTransaction - Fyntra API failed`,
          fyntraResponse,
        );
        throw new BadRequestException(
          new MessageResponseDto("Something went wrong"),
      ...
      paymentLink = fyntraResponse.payment_link;
      txnRefId = fyntraResponse?.txn_id;
      this.logger.info(
        `PAYIN - createTransaction - Fyntra API response: ${LoggerPlaceHolder.Json}`,
        fyntraResponse,
      );
    } catch (err: any) {
      this.logger.error(
        `PAYIN - createTransaction - Error calling Fyntra API`,
        err,
      );
      throw new BadRequestException("Failed to generate payment link");
    }
  ...
  async externalWebhookPayinFyntra(
    externalWebhookPayin: ExternalPayinWebhookOnikDto,
  ) {
    try {
  ...
  async externalWebhookPayinOnik(
    externalWebhookPayin: ExternalPayinWebhookOnikDto,
  ) {
    try {
      const { status, amount, rrn, txn_id } = externalWebhookPayin;
  ...
  async createPayoutRockyPayz(
    ...
    try {
      ...
      // Add to processing queue
      await this.payoutQueueRocky.add("process-rocky-payouts", {
        payoutOrders,
        userId: user.id,
        batchId,
      });
  ...
  async externalWebhookPayoutRockyPayz(
    webhookData: any,
  ): Promise<MessageResponseDto> {
    this.logger.info(
      `Rocky Webhook Data: ${LoggerPlaceHolder.Json}`,
      webhookData,
    );
    ...
    this.logger.info(`PAYOUT WEBHOOK - For OrderId: ${Txn_ID} :`, payOutOrder);
    ...
    if (payOutOrder.status === internalStatus) {
      return new MessageResponseDto(
        `Duplicate Webhook for PAYOUT/SETTLEMENT : ${Txn_ID}`,
      );
    }
    ...
    if (internalStatus === PAYMENT_STATUS.SUCCESS) {
      ...
      this.logger.info(
        `PAYOUT - Rocky Webhook - ${payOutOrder.id} - Webhook received successfully: ${LoggerPlaceHolder.Json}`,
        payOutOrderRaw,
      );
  ...
  async createNxtPayin(
    ...
    const payload: IExternalPayinPaymentRequestNxt = {
      amount,
      customer_mobile: mobile,
      customer_name: name,
    ...
    const { signature, timestamp, requestId } = getConfigNxt({
      payload,
      secret: nxt.secretKey,
    });

    const axiosServiceNxt = new AxiosService(NXT.BASE_URL, {
      headers: {
        "X-Client-ID": nxt.clientId,
    ...
    try {
      const nxtResponse =
        await axiosServiceNxt.postRequest<IExternalPayinPaymentResponseNxt>(
          NXT.PAYIN.LIVE,
          payload,
        );
      if (!nxtResponse || !nxtResponse.success) {
        this.logger.error(
          `PAYIN - createTransaction - Onik API failed`,
          nxtResponse,
        );
      ...
      paymentLink = nxtResponse.data?.upi_intent;
    } catch (err: any) {
      this.logger.error(
        `PAYIN - createTransaction - Error calling Onik API`,
        err,
